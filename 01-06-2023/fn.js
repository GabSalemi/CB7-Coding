...
export const postGen = (post) => {
    ...
    const postText = elementGen("p", "post__text", post.body.slice(0, 140))
...
/* export const followingGen = (userImg, userAlias, userName) => {
    const followDiv = elementGen("div", "follow__div")
    const followImg = elementGen("img", "follow__img", "", {name: "src", value: userImg})
    const followNameDiv = elementGen("div", "follow__names")
    const followAlias = elementGen("h3", "follow__alias", userAlias)
    const followName = elementGen("h4", "follow__name", userName)
...
export const addPost = (endpoint, title, userId) => {
